...
macro(cs_add_executable _target)
  ...
  # Prefix the target name with the project name if it isn't already
  if(NOT ${_target} MATCHES "^${PROJECT_NAME}.*")
    set(__target "${PROJECT_NAME}_${_target}")
  else()
    set(__target "${_target}")
  endif()
  cmake_parse_arguments(cs_add_executable_args "NO_AUTO_LINK;NO_AUTO_DEP" "" "" ${ARGN})
  add_executable(${__target} ${cs_add_executable_args_UNPARSED_ARGUMENTS})
  set_target_properties(${__target} PROPERTIES OUTPUT_NAME ${_target} PREFIX "")
  if(NOT cs_add_executable_args_NO_AUTO_LINK)
    target_link_libraries(${__target} ${catkin_LIBRARIES})
  endif()
  if(NOT cs_add_executable_args_NO_AUTO_DEP)
    if(NOT "${${PROJECT_NAME}_CATKIN_BUILD_DEPENDS_EXPORTED_TARGETS}" STREQUAL "")
      add_dependencies(${__target} ${${PROJECT_NAME}_CATKIN_BUILD_DEPENDS_EXPORTED_TARGETS})
    endif()
  endif()
  cs_add_targets_to_package(${__target})
endmacro()
...
macro(cs_add_library _target)
  ...
  add_library(${__target} ${cs_add_library_UNPARSED_ARGUMENTS})
  set_target_properties(${__target} PROPERTIES OUTPUT_NAME ${_target} PREFIX "")
  ...
  if(NOT cs_add_library_NO_AUTO_EXPORT)
    list(APPEND ${PROJECT_NAME}_LIBRARIES ${__target})
  endif()
  cs_add_targets_to_package(${__target})
endmacro()
